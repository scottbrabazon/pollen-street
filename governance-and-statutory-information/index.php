<!DOCTYPE html>

<html class="nojs" lang="en">

	<head>
		<meta charset="utf-8">
		<meta name="viewport" content="initial-scale=1.0,width=device-width" />
		<meta name="msvalidate.01" content="xxxx" />
		<title>Pollen Street</title>
		<meta name="description" content="xxxx" />
		<meta name="keywords" content="Keywords" />
		<link rel="stylesheet" media="all" href="/pollen-street/css/style.css" />
    
	    <!-- remove the no-js -->
	    <script>(function(H){H.className=H.className.replace(/\bnojs\b/,'js')})(document.documentElement)</script>

	</head>

	<body>

		<!-- Navigation -->
		<header id="the-firm-header">
			<div>
				<nav class="d-nav">
					<ul class="d-nav d-nav__list">
						<li class="d-nav__item"><a href="/pollen-street/">Home</a></li>
						<li class="d-nav__item"><a href="/pollen-street/the-firm">The Firm</a></li>
						<li class="d-nav__item d-nav__item"><a href="/pollen-street/private-equity">Private Equity</a></li>
						<li class="d-nav__item"><a href="/pollen-street/credit">Credit</a></li>
						<li class="d-nav__item"><a href="/pollen-street/people">People</a></li>
						<li class="d-nav__item"><a href="/pollen-street/portfolio">Portfolio</a></li>
						<li class="d-nav__item"><a href="/pollen-street/news">News</a></li>
						<li class="d-nav__item"><a href="/pollen-street/contact">Contact</a></li>
						<li class="d-nav__item"><a href="/pollen-street/login">Login</a></li>
					</ul>
				</nav>
			</div>
			<button class="burger-menu__open-button">
				<img src="/pollen-street/assets/img/burger-menu.svg" alt="" />
			</button>

	    </header>
	    <!-- End Navigation -->

		<!-- Intro -->
		<section id="private-equity-intro" class="private-equity-intro">
			<div class="intro">
				<a href="/pollen-street/">
					<img class="intro__logo" src="/pollen-street/assets/img/pollen-street-logo.svg" alt="" />
				</a>
				<div class="intro__header">
					<h1>Lorem ipsum dolor sit amet, mel legere suscipit.</h1>
				</div>
			</div>
		</section>
		<!-- End Intro -->

		<section id="governance-and-statutory" class="section">
			<div class="governance-and-statutory">
				<div class="governance-and-statutory__logo">
					<img src="/pollen-street/assets/img/pollen-street-logo-no-text.svg" alt="" />
				</div>
				<div class="governance-and-statutory__body">
					<p>Lorem ipsum dolor sit amet, mel legere suscipit eu, eu eum augue appetere. Ea accusam maluisset usu, mea ei affert corpora, ei dicant adipisci moderatius has. Sea ea animal menandri tincidunt. No sea postea labores corrumpit, pri facilis lobortis te, te modus posse sadipscing eam. Has facete omnesque cu, usu et sumo mediocrem similique.</p>

					<p>An vide blandit incorrupte pri, eos adhuc solet gubergren ei. Illum partiendo ius ea, has erat error accusam an. Tota natum no qui, assum senserit reprimique ex vix, quo ea rebum alterum commune. Ad sea exerci graecis disputando, quo natum movet concludaturque ne. No sea meliore feugait accumsan, postea aperiam consequat an usu.</p>

					<p>Ad falli principes suscipiantur pro. Ne vel error corrumpit. Pro odio torquatos cu. Modus discere in vis. Quo ad quod amet malorum, dictas aperiam assentior duo ei.</p>

					<p>Errem nobis ea has, cu tritani feugiat has, te errem phaedrum scripserit est. No pri justo clita fabulas, ius ex docendi patrioque. Nec facer dissentiet necessitatibus at. Usu ut invenire assentior quaerendum. Usu admodum inimicus at, et mazim graece principes mea.</p>

					<p>Tibique deseruisse ea mea. No quas iudico habemus sea, aperiri inciderint mei cu, sea ea atqui hendrerit adipiscing. Ne mea quodsi persequeris concludaturque. No sit recteque conclusionemque, quo ea malorum nusquam eloquentiam. Ea usu lorem dicam discere, similique temporibus te qui. Duo te adhuc iudico voluptatibus, te nisl ubique persius vis, inani veniam pri ea.</p>
				</div>
			</div>
		</section>

		<footer class="footer" id="footer">
			<div class="footer__container">
				<div class="footer__row">
					<img class="footer__footer-logo" src="/pollen-street/assets/img/pollen-street-logo-mixed.svg" alt="" />
					<p>Pollen Street Capital<br />8 Hanover Street<br />London W1S 1YQ</p>
				</div>
				<hr />
				<nav>
					<ul class="footer__footer-nav">
						<li><a href="/pollen-street/">Home</a></li>
						<li><a href="/pollen-street/the-firm">The Firm</a></li>
						<li class="footer__footer-nav--active"><a href="/pollen-street/private-equity">Private Equity</a></li>
						<li><a href="/pollen-street/credit">Credit</a></li>
						<li><a href="/pollen-street/people">People</a></li>
						<li><a href="/pollen-street/portfolio">Portfolio</a></li>
						<li><a href="/pollen-street/news">News</a></li>
						<li><a href="/pollen-street/contact">Contact</a></li>
						<li><a href="/pollen-street/login">Login</a></li>
						<li><a href="/pollen-street/governance-and-statutory-information">Governance &amp; Statutory Information</a></li>
					</ul>
				</nav>
				<hr />
				<a href="/pollen-street/governance-and-statutory-information">
					<p class="footer__governance-and-statutory-information"><strong>Governance &amp; Statutory Information</strong></p>
				</a>
				<p class="footer__disclaimer">&copy; <span id="date"></span> Pollen Street Capital. All Rights Reserved</p>
			</div>
		</footer>

		<div class="closed burger-menu" id="burger-menu">
			<div class="burger-menu__container">
				<div class="js-anim-single burger-menu__close-button"> 
					<img src="/pollen-street/assets/img/burger-menu-close-white.svg" alt="" />
				</div>
				<nav>
					<img class="burger-menu__logo" src="/pollen-street/assets/img/pollen-street-logo-mixed.svg" alt="" />
					<ul>
						<li><a href="/pollen-street/">Home</a></li>
						<li><a href="/pollen-street/the-firm">The Firm</a></li>
						<li><a href="/pollen-street/private-equity">Private Equity</a></li>
						<li><a href="/pollen-street/credit">Credit</a></li>
						<li><a href="/pollen-street/people">People</a></li>
						<li><a href="/pollen-street/portfolio">Portfolio</a></li>
						<li><a href="/pollen-street/news">News</a></li>
						<li><a href="/pollen-street/contact">Contact</a></li>
						<li><a href="/pollen-street/login">Login</a></li>
					</ul>
				</nav>
			</div>
		</div>
	    <script src="http://ajax.googleapis.com/ajax/libs/jquery/1.8.3/jquery.min.js"></script>
		<script src="/pollen-street/js/main.js"></script>
	</body>

</html>